...
const logPath = path.join(__dirname, "../logs/mentionFetchWorker.log")
    ...
    try {
      ...
      try {
        ...
        for (const tweet of tweets) {
          ...
          if (exists.length > 0) {
            logToFile(`Tweet sudah ada: ${tweetUrl}`)
            continue
          }
        ...
        logToFile(`Berhasil proses ${tweets.length} tweet untuk keyword "${keyword}"`)
      } catch (err) {
        logToFile(`Twitter error: ${err.message}`)
      }
      ...
      try {
        ...
        if (video) {
          ...
          for (const item of comments) {
            ...
            if (exists.length > 0) {
              logToFile(`Komentar YouTube sudah ada: ${comment.textDisplay}`)
              continue
            }
          ...
          logToFile(`Berhasil proses ${comments.length} comment pada youtube untuk keyword "${keyword}"`)
        }
      } catch (err) {
        logToFile(`YouTube error: ${err.message}`)
      }

      try {
        // SERPER.DEV
        const serperRes = await axios.post("https://google.serper.dev/news", { q: keyword }, {
          headers: {
        ...
        for (const article of articles) {
          ...
          if (exists.length > 0) {
            logToFile(`Artikel sudah ada: ${article.link}`)
            continue
          }
        ...
        logToFile(`Berhasil serper ${articles.length} artikel (Kompas & Medium) "${keyword}"`)
      } catch (err) {
        logToFile(`Serper error: ${err.message}`)
      }

    } catch (err) {
      logToFile(`Worker error: ${err.message}`)
    }
  }
}
...
setInterval(async () => {
  ...
  try {
    ...
    logToFile("Sentimen untuk seluruh mentions telah dianalisis.")
  } catch (err) {
    logToFile(`Gagal hit sentimen API: ${err.message}`)
  }
}, 30000) //setengah menit
